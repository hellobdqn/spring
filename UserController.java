...
public class UserController{
	...
	private List<User> qureyUserList = new ArrayList<User>();
	...
	public String list(@RequestParam(value="username",required=false)
						String username,Model model){
		logger.info("username=="+username+",获取list");
		if( null != username && !username.equals("")){
			for(User user:userList){
				if(user.getUserName().indexOf(username) != -1){
					qureyUserList.add(user);
				}
				model.addAttribute("qureyUserList",qureyUserList);
			}
				
		}else{
			model.addAttribute("qureyUserList",userList);
		}
		return "user/userlist";
	}
}
